...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="trandetails/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">商户交易时间</th>
						<th class="center">交易完成时间</th>
						<th class="center">商户订单号</th>
						<th class="center">批次号</th>
						<th class="center">代收付平台订单号</th>
						<th class="center">交易金额</th>
						<th class="center">币种</th>
						<th class="center">客户交易银行账号</th>
						<th class="center">客户交易银行账户名称</th>
						<th class="center">商户交易虚拟账号</th>
						<th class="center">交易结果码</th>
						<th class="center">交易结果描述</th>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>${var.TRAN_TYPE}</td>
										<td>${var.TRAN_DATE}</td>
										<td>${var.TRAN_DATED}</td>
										<td>${var.ORDER_ID}</td>
										<td>${var.NUM}</td>
										<td>${var.REP_ORDER_ID}</td>
										<td>${var.TRAN_AMOUT}</td>
										<td>${var.CURRENCY}</td>
										<td>${var.BANK_NO}</td>
										<td>${var.BANK_NAME}</td>
										<td>${var.USER_FIC_NO}</td>
										<td>${var.RESULT_NO}</td>
										<td>${var.RESULT_DESC}</td>
										<td>${var.POUND}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>trandetails/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>trandetails/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>trandetails/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>trandetails/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>trandetails/excel.do';
		}
		</script>
