...
public class AppUtil  {
	...
	public static boolean checkParam(String method, PageData pd){
		...
		if(falseCount>0){
			...
			for(int j=1;j<=falseCount;j++){
				logger.error(SessionUtil.getNo() + "   第"+j+"个："+ tempArray[j-1]);
			}
		} else {
			result = true;
		}
	...
	public static List<PageData>  setListDistance(List<PageData> list, PageData pd){
		...
		try{
			...
			logger.error(SessionUtil.getNo() + "缺失参数--user_longitude和user_longitude");
			logger.error(SessionUtil.getNo() + "lost param：user_longitude and user_longitude");
		}
