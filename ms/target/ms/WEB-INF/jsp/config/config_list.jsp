  ...
  <div class="row-fluid">
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">配置VALUE</th>
						<th class="center">配置类型</th>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td class="center">${var.TYPESTR}</td>
										<td  class='center'>${var.DESR}</td>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					...
					<a class="btn btn-small btn-success" onclick="reload();">重新加载</a>
					</c:if>
					<c:if test="${QX.del == 1 }">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function reload(){
			$.ajax({
				type: "POST",
				url: '<%=basePath%>config/reload.do?tm='+new Date().getTime(),
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>config/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>config/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>config/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>config/excel.do';
		}
		</script>
