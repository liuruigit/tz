...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="bankrule/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">银行简称</th>
						<th class="center">单笔限额</th>
						<th class="center">单日限额</th>
						<%--<th class="center">起始时间</th>--%>
						<%--<th class="center">结束时间</th>--%>
						<th class="center">渠道名称</th>
						<%--<th class="center">逻辑删除标志位：1正常0删除</th>--%>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center'>${vs.index+1}</td>
										<td>${var.BANK_NAME}</td>
										<td>${var.BANK_SHORT_NAME}</td>
										<td>${var.LIMIT}</td>
										<td>${var.DAY_LIMIT}</td>
										<%--<td><fmt:formatDate value="${var.TIME_LIMIT_BEGIN}" pattern="yyyy-MM-dd"/> </td>--%>
										<%--<td><fmt:formatDate value="${var.TIME_LIMIT_END}" pattern="yyyy-MM-dd"/></td>--%>
										<td>${var.CHANNEL_NAME}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>bankrule/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>bankrule/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>bankrule/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>bankrule/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>bankrule/excel.do';
		}
		</script>
