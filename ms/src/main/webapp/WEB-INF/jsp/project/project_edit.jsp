...
//		check("#AMOUNT");
		...
		var openDate = $("#OPEN_DATE").val().substring(0, 10);
		var endDate = $("#END_DATE").val().substring(0, 10);

		if(new Date(openDate.replace("-", "/").replace("-", "/")) >= new Date(endDate.replace("-", "/").replace("-", "/"))) {
			tail("#OPEN_DATE", "开始时间与结束时间不能为同一天，且开始时间小于结束时间！");
			$("#OPEN_DATE").focus();
			$("#END_DATE").val("");
			return false;
		}
			...
			<tr>
				...
				<td>
					<select name="RECOMMEND">
						<option <c:if test="${pd.RECOMMEND == 0}">selected</c:if> value="0">推荐</option>
						<option <c:if test="${pd.RECOMMEND == 1}">selected</c:if> value="1">不推荐</option>
					</select>
				</td>
			</tr>
			<tr>
				<td style="width:100px;text-align: right;padding-top: 13px;">VIP限制:</td>
				<td>
					<select name="VIP_LIMIT">
						<option <c:if test="${pd.VIP_LIMIT == 0}">selected</c:if> value="0">V0(普通用户)</option>
						<option <c:if test="${pd.VIP_LIMIT == 1}">selected</c:if> value="1">V1</option>
						<option <c:if test="${pd.VIP_LIMIT == 2}">selected</c:if> value="2">V2</option>
						<option <c:if test="${pd.VIP_LIMIT == 3}">selected</c:if> value="3">V3</option>
						<option <c:if test="${pd.VIP_LIMIT == 4}">selected</c:if> value="4">V4</option>
						<option <c:if test="${pd.VIP_LIMIT == 5}">selected</c:if> value="5">V5</option>
					</select>
				</td>
			</tr>
			<tr>
...
<script type="text/javascript">
	$(window).ready(function() {
		if(${empty msgType})
		$("#STATUS").next().text("该标的没有录入房产信息，不能出售，请尽快录入房产信息！")
	});
</script>
</body>
</html>