...
public class Money implements Serializable, Comparable {
     ...
     * 本货币对象的值不变。如果相乘后的金额不能转换为整数分，使用缺省的取整模式 <code>DEFUALT_ROUNDING_MODE</code>进行取整。
     *
     * @param val 乘数
     * @return 相乘后的结果。
     */
     ...
     * <p/>
     * 返回一个新的货币对象，币种与本货币对象相同，金额为本货币对象的金额除以除数。
     * 本货币对象的值不变。如果相除后的金额不能转换为整数分，使用缺省的取整模式 <code>DEFAULT_ROUNDING_MODE</code>进行取整。
     *
     * @param val 除数
     * @return 相除后的结果。
     */
    ...
    /**
     * 按指定的格式化字符串格式化. 如 10.20元, 用 # 格式时, 显示成 10; 用 #.0 格式化时, 显示 10.2.
     * 当不指定format时, 按数字格式化
     * 注意:这里的format指定的字符串格式要满足 DecimalFormat 类所接受的格式
     * @param format
     * @return
     */
    public String toStringWithFormat(String format) {
        if (format == null) {
            return NumberFormat.getNumberInstance().format(getAmount());
        }
        return new DecimalFormat(format).format(getAmount());
    }
     ...
     * @return
     */
    public static Money centToYuan(long cent) {
        Money money = new Money();
        money.setCent(cent);
        return money;
    }
    ...
    public static void main(String[] args) {
        System.out.println(new Money(1000).getCent() > 300000);
    }
}
