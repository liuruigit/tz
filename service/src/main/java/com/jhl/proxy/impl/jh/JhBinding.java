...
public class JhBinding extends BaseJh{
    ...
    protected JSONObject parseBizResp(JSONObject headJson, String xml) throws Exception {
        ...
        respJson.put("ACC_NO",JhClientMsg.getTxtInfo(xml,"ACC_NO"));//帐户号
        respJson.put("BALANCE",JhClientMsg.getTxtInfo(xml,"BALANCE"));//余额
        respJson.put("BALANCE1",JhClientMsg.getTxtInfo(xml,"BALANCE1"));//可用余额
        respJson.put("INTEREST",JhClientMsg.getTxtInfo(xml,"INTEREST"));//帐户利息
        respJson.put("INTEREST_RATE",JhClientMsg.getTxtInfo(xml,"INTEREST_RATE"));//帐户利率
        respJson.put("ACC_STATUS",JhClientMsg.getTxtInfo(xml,"ACC_STATUS"));//账户状态
        respJson.put("RESV_NAME1",JhClientMsg.getTxtInfo(xml,"RESV_NAME1"));//分行自定义输出名称1
        respJson.put("RESV1",JhClientMsg.getTxtInfo(xml,"RESV1"));//分行自定义输出内容1
        respJson.put("RESV_NAME2",JhClientMsg.getTxtInfo(xml,"RESV_NAME2"));//分行自定义输出名称2
        respJson.put("RESV2",JhClientMsg.getTxtInfo(xml,"RESV2"));//分行自定义输出内容2
