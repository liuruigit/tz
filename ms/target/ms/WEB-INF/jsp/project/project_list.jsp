				...
				<tr>
					...
					<td style="vertical-align:top;">
					 	<select class="chzn-select" name="STATUS" id="STATUS" data-placeholder="状态" class="select_top">
							...
							<option <c:if test="${pd.STATUS==5}">selected</c:if> value="5">已还款</option>
					  	</select>
					</td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">起投金额</th>
						<th class="center">累加金额</th>
						<th class="center">限投金额</th>
						<th class="center">预期年化收益率</th>
						<th class="center">服务费率</th>
						<th class="center">VIP限制</th>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center'><fmt:formatNumber value="${var.AMOUNT}" pattern="#,#00.00#"/></td>
								<td class='center'><fmt:formatNumber value="${var.FINAL_AMOUNT}" pattern="#,#00.00#"/></td>
								<td class='center'><fmt:formatNumber value="${var.SELLED_AMOUNT}" pattern="#,#00.00#"/></td>
								<td class='center'>${var.MIN}</td>
								<td class='center'>${var.STEP}</td>
								<td class='center'>${var.LIMIT}</td>
								<td class='center'>${var.ANNUAL_RATE}</td>
								<td class='center'>${var.SERVICE_RATE}</td>
								<td class='center'>${var.VIP_LIMIT}</td>
								<td class='center'>${var.OPEN_DATE}</td>
								<td class='center'>${var.END_DATE}</td>
								<td class='center'>${var.SOLD_OUT_DATE}</td>
									...
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											...
											<c:if test="${var.STATUS == 0}">
												<li><a style="cursor:pointer;" title="更改状态" onclick="upd('${var.NO}', '${var.ID}');" class="tooltip-warning" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-pencil"></i></span></a></li>
											</c:if>
											<c:if test="${QX.del == 1 }">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function upd(no, id) {
			bootbox.confirm("标的状态将被改为发布?", function(result) {
				if(result) {
					//top.jzts();
					var url = "<%=basePath%>project/updateStatus.do?NO="+no+"&ID="+id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						//bootbox.alert(data.message);
						data = data.code;
						if(data == 0) {
							bootbox.alert("请添加房产信息后修改状态！");
						} else {
							nextPage(${page.currentPage});
						}
