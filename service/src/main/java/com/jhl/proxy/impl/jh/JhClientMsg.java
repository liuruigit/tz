 ...
 * 金运通连接工具类
 */
public class JhClientMsg {

    protected static final Logger log = Logger.getLogger(JhClientMsg.class);
    //测试环境       外部访问地址			http://test1.jytpay.com:8080/JytCPService/tranCenter/encXmlReq.do
    protected static String APP_SERVER_URL = "http://test1.jytpay.com:20080/JytAuth/tranCenter/authReq.do";

    //测试环境测试商户
    protected static String MERCHANT_ID = "290041120001";	//替换为自己的商户号
    ...
    protected static String CLIENT_PRIVATE_KEY="MIIEvgIBADANBgkqhkiG9w0BAQEFAASCBKgwggSkAgEAAoIBAQDIAuGmpyZPfS9Y"+
    ...
    //替换匹配的服务端公钥
    //测试环境
    protected static String SERVER_PUBLIC_KEY="MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAz9rX1JG51udh5L6Co1od"+
...
//    }
    ...
    protected static String sendAuthMsg(String xml,String sign) throws Exception{
        ...
        return sendMsg(xml, sign, "http://test1.jytpay.com:20080/JytAuth/tranCenter/authReq.do");
    }
    ...
    public String sendTranMsg(String xml,String sign) throws Exception{
        ...
        return sendMsg(xml,sign,"http://test1.jytpay.com:8080/JytCPService/tranCenter/encXmlReq.do");
    }
    ...
    public static void main(String[] args) {
        String sdaio = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" +
                "<message>" +
                "<head>" +
                "<version>1.0.0</version><tran_type>02</tran_type><merchant_id>290041120001</merchant_id>" +
                "<tran_date>20160220</tran_date><tran_time>223915</tran_time><tran_flowid>290041120001648235693129793557" +
                "</tran_flowid><tran_code>TR4002</tran_code><resp_code>ER001000</resp_code><resp_desc>银联验证失败" +
                "！</resp_desc></head><body></body></message>";
        try {
            System.out.println("jX5s2O4c5HZqSod8GUPqpQ==".length());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    protected static JSONObject getRespMsgJson(String respMsg, JSONObject jsonObject) {
        try {
            if(getBodyNode(respMsg,"TX_CODE").equals("")) {

            } else if(getBodyNode(respMsg,"TX_CODE").equals("")){
