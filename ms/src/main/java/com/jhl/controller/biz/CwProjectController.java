 ...
 */
@Controller
@RequestMapping(value="/cwProject")
public class CwProjectController extends BaseController {

	String menuUrl = "cwProject/list.do"; //菜单地址(权限用)
	@Resource(name="projectService")
	private com.jhl.service.ProjectService projectService;
	@Resource(name="msProjectService")
	private ProjectService msprojectService;
	 ...
	 * 发布改为满标
	 */
	@RequestMapping(value = "/completeSub")
	@ResponseBody
	public JsonBack completeSub() {
		logBefore(logger, "满标状态的修改");
		JsonBack jsonBack = new JsonBack();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			Integer proId = Integer.parseInt(pd.getString("ID"));
			Project project = projectService.queryById(proId);
			boolean flag = transactionService.ensure(project);
			if(flag) {
				jsonBack.setCode(0);//满标成功
			} else {
				jsonBack.setCode(1);//满标失败
			}
		} catch (Exception e) {
			logger.error(SessionUtil.getNo() + "操作异常！", e);
			jsonBack.setCode(2);//操作失败
		}
		return jsonBack;
	}
	...
	@ResponseBody
	public JsonBack repayment() {
		logBefore(logger, "还款状态的修改");
		...
		try {
			Integer proId = Integer.parseInt(pd.getString("ID"));
			Project project = projectService.queryById(proId);
			transactionService.settleProject(project);
			jsonBack.setCode(0);
		} catch (Exception e) {
			logger.error(SessionUtil.getNo() + "操作异常！", e);
			jsonBack.setCode(1);
...
//		pd.put("PROJECT_ID", this.get32UUID());	//主键
	...
	public ModelAndView list(Page page){
		...
		try{
			...
			mv.setViewName("cwproject/cwproject_list");
	...
	JsonBack findByNo(Page page){
		...
		try{
			pd = this.getPageData();
			page.setPd(pd);
			PageData obj = msprojectService.findByNo(pd);	//列出Project列表
			jsonBack.setObj(obj);
	...
	public JsonBack updateStatus() throws Exception {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		msprojectService.updateStatus(pd);
	...
	public JsonBack deleteStatus() {
		...
		try{
			pd = this.getPageData();
			msprojectService.updateDeleteStatus(pd);//修改逻辑删除位
	...
	public Object deleteAll() {
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				msprojectService.deleteAll(ArrayDATA_IDS);
