  ...
  <div class="row-fluid">
			...
			<table>
				<tr>
					...
					<td style="vertical-align:top;"> 
					 	<select class="chzn-select" name="status" id="field2" data-placeholder="请选择" style="vertical-align:top;width: 120px;">
							<option value=""></option>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function edit(ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="处理结果";
			 diag.URL = '<%=basePath%>feedback/goAdd?ID='+ID;
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>feedback/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function add(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>feedback/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>feedback/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>feedback/excel.do';
		}
		</script>
