...
public class CouponController extends BaseController {
	...
	public ModelAndView exportExcel(){
		...
		try{
			...
			titles.add("截止日期");	//5
			titles.add("适用范围");	//6
			titles.add("逻辑删除标志位：1正常0删除");	//7
			titles.add("逻辑删除标志位：1正常0删除");	//8
			dataMap.put("titles", titles);
			List<PageData> varOList = couponService.listAll(pd);
			List<PageData> varList = new ArrayList<PageData>();
			for(int i=0;i<varOList.size();i++){
				PageData vpd = new PageData();
				vpd.put("var1", varOList.get(i).getString("DESC"));	//1
				vpd.put("var2", varOList.get(i).getString("NAME"));	//2
				vpd.put("var3", varOList.get(i).getString("MIN"));	//3
				vpd.put("var4", varOList.get(i).getString("BEGIN_DATE"));	//4
				vpd.put("var5", varOList.get(i).getString("END_DATE"));	//5
				vpd.put("var6", varOList.get(i).getString("TAG"));	//6
				vpd.put("var7", varOList.get(i).getString("DELETE_STATE"));	//7
