package com.jhl.controller.system.head;
...
import com.jhl.service.system.appuser.AppuserService;
import com.jhl.service.system.user.UserService;
import com.jhl.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",jhl,"+pd.getString("PORT")+",jhl,"+pd.getString("EMAIL")+",jhl,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",jhl,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",jhl,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",jhl,"+pd.getString("fcontent")+",jhl,"+pd.getString("fontSize")+",jhl,"+pd.getString("fontX")+",jhl,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",jhl,"+pd.getString("imgUrl")+",jhl,"+pd.getString("imgX")+",jhl,"+pd.getString("imgY"));	//图片水印配置
		WaterMarkUtil.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
